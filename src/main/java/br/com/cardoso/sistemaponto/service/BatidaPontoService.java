...
public class BatidaPontoService {
    ...
    public Iterable obtemPontos() {
        return batidaPontoRepository.findAll();
    }

    public ControleHoras obtemPontoPorUsuario(Long id) {
        ...
        for (int i = 0; i < batidasPonto.size(); i++) {
            BatidaPonto batidaAtual = batidasPonto.get(i);
            if (batidaAtual.isEntrada()) {
                int nextIndex = i + 1;
                boolean hasNextItem = nextIndex < batidasPonto.size();

                DateTime entrada = batidaAtual.getDataBatida();
                DateTime saida = new DateTime();

                if (hasNextItem) {
                    BatidaPonto batidaSaida = batidasPonto.get(nextIndex);
    ...
    public void deletaPonto(Long id) {
